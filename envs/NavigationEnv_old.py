...
class NavigationEnv(DroneGymEnvsBase):
    ...
    def get_success(self) -> th.Tensor:
        reached_target = (self.position - self.target).norm(dim=1) <= self.success_radius
        episode_ended = self._step_count >= self.max_episode_steps
        return reached_target & episode_ended
