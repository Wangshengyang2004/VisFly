...
class NavigationEnv(DroneGymEnvsBase):
    def __init__(
        ...
        target_pos = [9, 0.0, 1] if target is None else target
    ...
    def get_observation(self, indices=None) -> Dict:
        ...
        accel = self.acceleration.to(self.device)
    ...
    def get_reward(self) -> th.Tensor:
        target_vec = self.target - self.position
        target_dis = target_vec.norm(dim=1)

        # 避障
        collision_dis = self.collision_vector.norm(dim=1)
        obstacle_approaching_v = (self.collision_vector * self.velocity).sum(dim=1) / (collision_dis + 1e-8)
        r_obstacle_vel_avoid = -obstacle_approaching_v.norm()* th.relu(1 - collision_dis) * 0.01 # TODO: Check if velocity is necessary
        # 目标
        r_distance_penalty = -0.005 * target_dis
        # 角速度惩罚
        r_ang_vel_penalty = self.angular_velocity.norm(dim=1) * -0.01
        r_ang_acc_penalty = self.angular_acceleration.norm(dim=1) * -0.001
        r_acc_penalty = self.acceleration.norm(dim=1) * -0.01
        r_collision_distance_penalty = -0.01 * collision_dis
        # 速度与方向对齐奖励
        velocity_norm = self.velocity.norm(dim=1) + 1e-6
        alignment = (self.direction * self.velocity).sum(dim=1) / velocity_norm
        r_view_align = alignment * 0.1
        
     
        reward = (r_distance_penalty + 
                r_obstacle_vel_avoid + 
                r_ang_vel_penalty + 
                r_acc_penalty +
                r_ang_acc_penalty +
                r_collision_distance_penalty +
                r_view_align)
        # Metrics for logging/debugging
        metrics = {
            "reward": reward,
            "distance_penalty": r_distance_penalty.detach(),
            "obstacle_vel_avoid": r_obstacle_vel_avoid.detach(),
            "ang_vel_penalty": r_ang_vel_penalty.detach(),
            "ang_acc_penalty": r_ang_acc_penalty.detach(),
            "acc_penalty": r_acc_penalty.detach(),
            "view_align": r_view_align.detach(),
            "collision_distance_penalty": r_collision_distance_penalty.detach(),
            "target_distance_min": target_dis.detach().mean()
        }
...
class NavigationEnv2(DroneGymEnvsBase):
    def __init__(
        ...
        if target is None:
            target_tensor = th.tensor([9, 0.0, 1], device=self.device)
        else:
    ...
    def get_success(self) -> th.Tensor:
        return (self.position.to(self.device) - self.target.to(self.device)).norm(
            dim=1
        ) <= self.success_radius
    ...
    def get_observation(self, indices=None) -> Dict:
        ...
        if (
            self.sensor_kwargs
            and len(self.sensor_kwargs) > 0
            and "depth" in self.sensor_obs
        ):
    ...
    def get_analytical_reward(
        ...
        target_approaching_v, target_away_v, target_dis = get_along_vertical_vector(
            self.target - dyn.position, dyn.velocity
        )
        ...
        view_aware_r = (
            (
                (dyn.direction * dyn.velocity).sum(dim=1)
                / (1e-6 + dyn.velocity.norm(dim=1))
            )
            .clamp(-1.0, 1.0)
            .acos()
            - thrd_perce
    ...
    def get_reward(self) -> Union[th.Tensor, Dict[str, th.Tensor]]:
        ...
        thrd_perce = th.pi / 18
        target_approaching_v, target_away_v, target_dis = get_along_vertical_vector(
            self.target - position_safe, velocity_safe
        )
        obstacle_approaching_v, obstacle_away_v, collision_dis = (
        ...
        view_aware_r = (
            (
                (direction_safe * velocity_safe).sum(dim=1)
                / (1e-6 + velocity_safe.norm(dim=1))
            )
            .clamp(-1.0, 1.0)
            .acos()
            - thrd_perce
...
class NavigationEnv3(NavigationEnv):
    ...
    def __init__(
        ...
        for kw in random_kwargs["state_generator"]["kwargs"]:
            kw["orientation"] = {"mean": [0.0, 0.0, 0.0], "half": [0.0, 0.0, pi]}
        ...
        self.max_target_distance = (
            15.0  # Maximum expected target distance for normalization
        )
        ...
        sim_device = th.device(
            "cpu"
        )  # Will be updated in reset to match position device
        self.prev_velocity = th.zeros(
            (self.num_envs, 3), dtype=th.float32, device=sim_device
        )
        self.prev_acceleration = th.zeros(
            (self.num_envs, 3), dtype=th.float32, device=sim_device
        )
        ...
        if (
            self.visual
            and hasattr(self.envs, "sceneManager")
            and self.envs.sceneManager is not None
        ):
    ...
    def generate_collision_free_spawn_positions(
        ...
        valid_positions = th.zeros(
            (num_positions, 3), dtype=th.float32, device=self.device
        )
        valid_count = 0
        ...
        for _attempt in range(max_attempts):
            ...
            random_positions = th.rand(
                remaining, 3, dtype=th.float32, device=self.device
            )
            ...
            if len(valid_indices) > 0:
                ...
                valid_positions[valid_count:end_idx] = random_positions[
                    valid_indices[: end_idx - valid_count]
                ]
                valid_count = end_idx

        if valid_count < num_positions:
            print(
                f"Warning: Only found {valid_count}/{num_positions} collision-free spawn positions"
            )
            ...
            for i in range(valid_count, num_positions):
                if valid_count > 0:
                    ...
                    valid_positions[i] = th.tensor(
                        [0.0, 0.0, 1.0 + i * 0.1], device=self.device
                    )
    ...
    def validate_target_position(self, target_positions: th.Tensor) -> th.Tensor:
        ...
        invalid_targets = self.envs.sceneManager.get_point_is_collision(
            std_positions=target_positions, scene_id=0, uav_radius=self.drone_radius
        )
    ...
    def generate_valid_targets(
        ...
        for _attempt in range(max_attempts):
            ...
            distance = min_distance + (max_distance - min_distance) * th.rand(
                num_targets, device=self.device
            )
            start_indices = (
                th.arange(num_targets, device=self.device) % start_positions.shape[0]
            )
            ...
            if valid_targets.all():
                return targets
        print(
            f"Critical: Could not find valid targets after {max_attempts} attempts. Returning last attempt."
        )
        return targets
    ...
    def set_valid_targets(self, new_targets: th.Tensor) -> bool:
        ...
        if valid_targets.all():
            ...
            print(
                f"Warning: {valid_targets.sum().item()} out of {len(valid_targets)} targets are invalid"
            )
            return False

    def set_valid_targets_with_retry(
        self, new_targets: th.Tensor, start_positions: Optional[th.Tensor] = None
    ) -> bool:
        """
        ...
        if valid_targets.all():
            ...
            print(
                f"Warning: {valid_targets.sum().item()} out of {len(valid_targets)} targets are invalid, attempting retry..."
            )
            ...
            if start_positions is not None:
                ...
                if len(valid_replacement_targets) < num_targets_needed:
                    ...
                    valid_replacement_targets = th.cat(
                        [valid_replacement_targets, padding], dim=0
                    )

                # Validate the replacement targets
                replacement_validation = self.validate_target_position(
                    valid_replacement_targets
                )
                ...
                if replacement_validation.all():
                    ...
                    print(
                        f"Warning: {replacement_validation.sum().item()} out of {len(replacement_validation)} replacement targets still invalid"
                    )

            # If retry failed, use fallback targets
            if start_positions is not None:
                fallback_targets = self.generate_fallback_targets(
                    start_positions, 2.0, 8.0
                )
    ...
    def generate_valid_targets_with_retry(
        ...
        for batch in range(max_batches):
            ...
            for _attempt in range(max_attempts_per_batch):
                ...
                distance = min_distance + (max_distance - min_distance) * th.rand(
                    len(invalid_indices), device=self.device
                )
            ...
            if batch == max_batches - 1 and not valid_mask.all():
                print(
                    f"Warning: Could not find valid targets for {actual_num_targets - valid_mask.sum().item()} positions after {max_batches} batches"
                )

        return targets

    def generate_fallback_targets(
        self,
        start_positions: th.Tensor,
        min_distance: float = 2.0,
        max_distance: float = 8.0,
    ...
    def _reset_tracking_buffers(self):
        ...
        with th.no_grad():
            ...
            if hasattr(self, "target") and self.target is not None:
                ...
                self.prev_target_distance = (
                    (current_target - current_pos).norm(dim=1).to(sim_dev)
                )
    ...
    def get_observation(self):
        ...
        assert angular_velocity.shape == (N, 3), (
            f"angular_velocity shape: {angular_velocity.shape}"
        )
    ...
    def reset(self, *args, **kwargs):
        ...
        if (
            ...
            scenes_loaded = sum(
                1 for scene in self.envs.sceneManager.scenes if scene is not None
            )
            if scenes_loaded == 0:
                ...
                print(
                    f"Warning: Only {scenes_loaded}/{self.envs.sceneManager.num_scene} scenes loaded"
                )
        ...
        if not spawn_valid.all():
            ...
            if not self.requires_grad:
                print(
                    f"Warning: {spawn_valid.sum().item()}/{len(spawn_valid)} spawn positions are collision-free"
                )
                print("Regenerating spawn for invalid agents...")
            invalid_indices = th.where(~spawn_valid)[0]
            new_spawn_positions = self.generate_collision_free_spawn_positions(
                num_positions=self.num_envs
            )
            # Properly reset only the invalid agents using the API
            self.reset_agent_by_id(
                agent_indices=invalid_indices,
                state=(
    ...
    def get_analytical_reward_components(
        ...
        if vel_magnitude.max() > 1e-6:
            ...
            r_alignment = (
                self.lambda_p * 0.3 * alignment
            )  # Increased weight for alignment
        else:
        ...
        safety_margin = (
            self.drone_radius * 3.0
        )  # Reduced from 5.0 for less aggressive penalty
...
class NavigationEnv4(DroneGymEnvsBase):
    def __init__(
        self,
        num_agent_per_scene: int = 1,
        num_scene: int = 1,
    ...
    def get_reward(self):
        ...
        reward = (
            (
                (velocity * (target - position)).sum(dim=1)
                / (1e-6 + (target - position).norm(dim=1))
            ).clamp_max(10)
            * 0.01
            + (
                ((direction * velocity).sum(dim=1) / (1e-6 + velocity.norm(dim=1)) / 1)
                .clamp(-1, 1)
                .acos()
            ...
            * 0.01
            + (orientation - th.tensor([1, 0, 0, 0], device=self.device)).norm(dim=1)
            * -0.00001
            + velocity.norm(dim=1) * -0.002
            + angular_velocity.norm(dim=1) * -0.001
            + (1 / collision_dis + 0.2) * -0.01
            + (1 - collision_dis).relu()
            * (
                (collision_vector * velocity).sum(dim=1) / (1e-6 + velocity.norm(dim=1))
            ).relu()
            * -0.005
            + self._success
            * (self.max_episode_steps - self._step_count)
            * base_r
            * (0.2 + 0.8 / (1 + 1 * velocity.norm(dim=1)))
        )

        if self.tensor_output:
            # Fill in the dict with actual reward components
            # r_position: reward for moving toward the target (progress along target direction)
            r_position = (
                (velocity * (target - position)).sum(dim=1)
                / (1e-6 + (target - position).norm(dim=1))
            ).clamp_max(10) * 0.01

            # r_alignment: penalty for misalignment between velocity and direction
            r_alignment = (
                ((direction * velocity).sum(dim=1) / (1e-6 + velocity.norm(dim=1)))
                .clamp(-1, 1)
                .acos()
                .clamp_min(thrd_perce)
                - thrd_perce
            ...
            # r_orientation: penalty for deviation from reference orientation
            r_orientation = (orientation - th.tensor([1, 0, 0, 0], device=self.device)).norm(dim=1) * -0.00001

            # r_velocity: penalty for high velocity
            r_velocity = velocity.norm(dim=1) * -0.002

            # r_angular_velocity: penalty for high angular velocity
            r_angular_velocity = angular_velocity.norm(dim=1) * -0.001

            # r_obstacle: penalty for proximity to obstacles (collision distance)
            r_obstacle = (1 / collision_dis + 0.2) * -0.01

            # r_collision: penalty for moving into obstacles
            r_collision = (1 - collision_dis).relu() * (
                (collision_vector * velocity).sum(dim=1) / (1e-6 + velocity.norm(dim=1))
            ).relu() * -0.005

            # r_success: bonus for success
            r_success = self._success * (self.max_episode_steps - self._step_count) * base_r * (0.2 + 0.8 / (1 + 1 * velocity.norm(dim=1)))

            # r_progress: for this reward, we can use r_position as a proxy (or set to zero if not tracked)
            r_progress = r_position
...
class NavigationEnv5(DroneGymEnvsBase):
    def __init__(
        self,
        num_agent_per_scene: int = 1,
        num_scene: int = 1,
    ...
    def get_reward(self) -> th.Tensor:
        ...
        # ang_vel = self.angular_velocity.clone()
        target_distance = (self.target - self.position).norm(dim=1).detach()
        target_vector = (self.target - self.position) / (
            1e-6 + target_distance.unsqueeze(1)
        )
        # Option 1: Your current approach (creates sharp transition)
        # if target_distance > 1:
        #     scale = target_distance
        # else:
        #     scale = 1
        # target_r = th.exp(-0.2 * target_distance) / scale

        # Option 2: Smoother scaling (recommended)
        # Uses (1 + distance) for smooth scaling everywhere
        target_r = th.exp(-0.2 * target_distance) / (1 + target_distance)

        # Option 3: Your friend's exact requirement (if you want to keep it)
        # scale = th.where(target_distance > 1, target_distance, th.ones_like(target_distance))
        # target_r = th.exp(-0.2 * target_distance) / scale

        # Approach velocity reward - reward for moving toward target
        approach_velocity = (vel * target_vector).sum(dim=1).clamp(min=0)
        approach_velocity_reward = approach_velocity * 1.5

        # 3. Forward direction alignment - reward looking toward target
        forward_dir = self.direction.clone()  # drone's forward direction
        target_alignment = (forward_dir * target_vector).sum(dim=1)
        direction_reward = target_alignment.clamp(min=0) * 0.5

        # Hovering stability reward - reward low velocity when close to target
        hover_radius = 2.0
        in_hover_zone = (target_distance <= hover_radius).float()
        stability_reward = in_hover_zone * th.exp(-vel.norm(dim=1)) * 1.0

        # Altitude penalty - penalize flying too high
        altitude = self.position[:, 2]
        altitude_limit = 2.5
        altitude_penalty = th.where(
            altitude > altitude_limit,
            -2.0
            * (altitude - altitude_limit) ** 2,  # Quadratic penalty for flying too high
            0.0,
        )

        collision_distance = self.collision_dis.clone().clamp(min=1e-6, max=10.0)
        safety_margin = 2.0
        collision_penalty = th.where(
            collision_distance < safety_margin,
            -3.0 * th.exp(-(collision_distance / 0.5)),
            0.0,
        )

        # 7. Penalty for velocity toward obstacles
        collision_vector = self.collision_vector.clone()
        collision_approach_penalty = th.where(
            (collision_vector * vel).sum(dim=1) > 0,
            -0.3 * (collision_vector * vel).sum(dim=1),
            0.0,
        )

        reward = (
            target_r
            + approach_velocity_reward
            + direction_reward
            + stability_reward
            + altitude_penalty
            + collision_penalty
            + collision_approach_penalty
        )
    ...
    def get_failure(self) -> th.Tensor:
        """Episodes end on collision or altitude violation."""
        altitude_violation = (self.position[:, 2] > 3) | (
            self.position[:, 2] < 0.2
        )  # Terminate if altitude > 3m or < 0.2m
        return self.is_collision | altitude_violation
